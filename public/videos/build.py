...
W, H   = 1280, 720
FPS    = 24
DUR    = 11   # seconds per scene → 7×11 + 9 packshots - overlaps ≈ 1:20
...
SCENES = [
    {
        "source": "v1",  # Mumbai crowd video
        "line1": [("En 2023,", False), ("l'Inde", False), ("a dépassé", False), ("la Chine.", False)],
        "line2": [("1,44 milliard", True), ("d'êtres humains.", False)],
        "sub":   "Le plus grand pays du monde par la population."
    },
    {
        "source": "p1",  # Bangalore photo
    ...
    {
        "source": "p2",  # UN conference photo
        "line1": [("Ni pro-Occident.", False), ("Ni pro-Russie.", False)],
        "line2": [("L'Inde achète", False), ("le pétrole russe", True), ("et négocie avec Washington.", False)],
        "sub":   "Une doctrine : l'intérêt national avant tout."
    },
    {
        "source": "v2",  # Split India video
        "line1": [("Mais", False), ("600 millions", True), ("d'Indiens", False)],
        "line2": [("vivent encore sous le seuil de pauvreté.", False)],
        "sub":   "Le défi intérieur est aussi grand que l'ambition mondiale."
    },
    {
        "source": "v1",  # Reuse Mumbai crowd — space/tech scene
        "line1": [("En 2023,", False), ("l'Inde a posé un rover", False), ("sur la Lune.", True)],
        "line2": [("Budget :", False), ("10 fois moins cher", True), ("que la NASA.", False)],
        "sub":   "L'efficacité comme doctrine technologique."
    },
    {
        "source": "v2",  # Reuse split — demographics
...
def render_packshot_outro(t, total=5.0):
    ...
    if t < 0.8:
        ov = Image.new("RGBA",(W,H),(0,0,0,int((1-t/0.8)*255)))
        img = Image.alpha_composite(img, ov)
        draw = ImageDraw.Draw(img)

    p0 = min(1.0,max(0,(t-0.2)/0.6))
    draw.rectangle([0,0,int(W*easeOut(p0)),4],fill=OR)

    if t > 0.5:
        p = min(1.0,(t-0.5)/0.9); a = int(easeOut(p)*255)
        bb = draw.textbbox((0,0),"PRISME",font=f_big)
        x  = cx-(bb[2]-bb[0])//2
        draw.text((x,H//2-90),"PRISME",font=f_big,fill=(*BLANC,a))

    if t > 1.2:
        p = min(1.0,(t-1.2)/0.5); a = int(easeOut(p)*255)
        lw = int(easeOut(p)*160)
        draw.rectangle([cx-lw//2,H//2+0,cx+lw//2,H//2+2],fill=(*OR,a))

    if t > 1.7:
        p = min(1.0,(t-1.7)/0.7); a = int(easeOut(p)*200)
        sl = int((1-easeOut(p))*12)
    ...
    if t > 2.4:
        p = min(1.0,(t-2.4)/0.5); a = int(easeOut(p)*180)
